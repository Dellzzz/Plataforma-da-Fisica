/* ============================================================
   ...
   Um só botão flutuante que abre um painel com:
     ...
     • Alto contraste     (baixa visão / ambiente claro)
     • Modo foco          (uma seção por vez; só nas aulas)
     • Ler em voz alta    (voz do navegador, em pt-BR)
     • Tamanho do texto   (A- / A / A+)
     • Fonte de leitura   (Padrão / Leitura / OpenDyslexic)
   ...
   Observações:
   - A opção "OpenDyslexic" baixa uma fonte externa (precisa de
     internet); se falhar, cai numa fonte legível do sistema.
   - As preferências ficam salvas e valem em toda a plataforma.
   ============================================================ */
(function () {
  'use strict';
  ...
  if (estado.fonte === undefined) estado.fonte = 1;        // tamanho (zoom)
  if (estado.hc === undefined) estado.hc = false;          // alto contraste
  if (estado.font === undefined) estado.font = 'padrao';   // família da fonte
  ...
  function aplicarContraste() {
    document.documentElement.classList.toggle('ax-hc', !!estado.hc);
    pintar();
  }

  /* ---------- 6) FONTE (leitura / dislexia) ---------- */
  function aplicarFamilia() {
    var d = document.documentElement;
    d.classList.toggle('ax-font-leitura', estado.font === 'leitura');
    d.classList.toggle('ax-font-dis', estado.font === 'dis');
    if (elFamBtns) for (var i = 0; i < elFamBtns.length; i++) {
      var b = elFamBtns[i];
      b.classList.toggle('ax-fon-on', b.getAttribute('data-font') === estado.font);
    }
  }
  ...
  '#axPanel{position:absolute;right:0;bottom:62px;width:236px;max-height:82vh;overflow-y:auto;' +
  'background:#0f1728;border:1px solid rgba(255,255,255,.14);border-radius:16px;padding:10px;' +
  ...
  /* seletor de fonte */
  '.ax-fam{display:flex;flex-direction:column;gap:5px;padding:9px 12px;border-radius:10px;' +
  'background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08)}' +
  '.ax-fam .ax-fam-h{font-family:Rajdhani,system-ui,sans-serif;font-weight:600;font-size:13px;color:#7fa6bf}' +
  '.ax-fam button{width:100%;text-align:left;padding:9px 11px;border-radius:8px;' +
  'border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.03);color:#dbe6f5;' +
  'font-size:15px;font-weight:600;cursor:pointer}' +
  '.ax-fam button:hover{border-color:rgba(0,229,255,.4)}' +
  '.ax-fam button.ax-fon-on{background:#00e5ff;color:#04121a;border-color:#00e5ff}' +
  '#axRoot .ax-fam .f-padrao{font-family:Rajdhani,system-ui,sans-serif!important}' +
  '#axRoot .ax-fam .f-leitura{font-family:Verdana,Tahoma,"Segoe UI",system-ui,sans-serif!important}' +
  '#axRoot .ax-fam .f-dis{font-family:"OpenDyslexic",Verdana,Tahoma,sans-serif!important}' +
  /* ---- MODO CALMO ---- */
  ...
  /* ---- ALTO CONTRASTE ---- */
  'html.ax-hc{--bg:#000;--bg-2:#000;--panel:#000;--panel-solid:#000;--txt:#fff;--txt-dim:#f2f2f2;' +
  '--cyan:#00ffff;--cyan-dim:#00ffff;--aqua:#00ffff;--blue:#66ccff;--violet:#e6b3ff;--magenta:#ff77ff;' +
  '--green:#00ff7f;--yellow:#ffff00;--gold:#ffff00;--orange:#ffb033;--red:#ff6b6b;' +
  '--line:#ffffff;--line-soft:rgba(255,255,255,.55)}' +
  'html.ax-hc body{background:#000!important;color:#fff!important}' +
  'html.ax-hc body::before,html.ax-hc body::after{display:none!important}' +
  'html.ax-hc *{text-shadow:none!important}' +
  'html.ax-hc .card,html.ax-hc .sim,html.ax-hc .sim-head,html.ax-hc .sim-controls,html.ax-hc .enem-q,' +
  'html.ax-hc .eq-head,html.ax-hc .eq-body,html.ax-hc .eq-alt,html.ax-hc .note,html.ax-hc .formula,' +
  'html.ax-hc .map,html.ax-hc .map .m,html.ax-hc .laws .lw,html.ax-hc .sheet .f,html.ax-hc .topbar,' +
  'html.ax-hc .toc-panel,html.ax-hc .readout .ro,html.ax-hc .chip{background:#000!important;border-color:#fff!important}' +
  'html.ax-hc p,html.ax-hc li,html.ax-hc .lead,html.ax-hc .eq-enun,html.ax-hc .eq-sol p,html.ax-hc .note,' +
  'html.ax-hc .map .d,html.ax-hc .laws .d,html.ax-hc .sheet .ds,html.ax-hc .eq-alt{color:#fff!important}' +
  'html.ax-hc a{color:#00ffff!important}' +
  /* ---- FONTES ALTERNATIVAS ---- */
  '@font-face{font-family:"OpenDyslexic";font-display:swap;font-weight:400;' +
  'src:url("https://cdn.jsdelivr.net/npm/@fontsource/opendyslexic/files/opendyslexic-latin-400-normal.woff2") format("woff2")}' +
  '@font-face{font-family:"OpenDyslexic";font-display:swap;font-weight:700;' +
  'src:url("https://cdn.jsdelivr.net/npm/@fontsource/opendyslexic/files/opendyslexic-latin-700-normal.woff2") format("woff2")}' +
  'html.ax-font-leitura body,html.ax-font-leitura body *{font-family:Verdana,Tahoma,"Segoe UI",system-ui,sans-serif!important}' +
  'html.ax-font-dis body,html.ax-font-dis body *{font-family:"OpenDyslexic",Verdana,Tahoma,sans-serif!important}' +
  'html.ax-font-leitura body,html.ax-font-dis body{letter-spacing:.03em;word-spacing:.07em;line-height:1.75!important}' +
  '#axRoot .ax-fam .f-padrao,#axRoot .ax-fam .f-leitura,#axRoot .ax-fam .f-dis{letter-spacing:normal!important;word-spacing:normal!important}';
  ...
  h += '<button class="ax-item" id="axContraste" type="button" role="menuitemcheckbox" aria-checked="false">' +
       '<span>\uD83C\uDF13 Alto contraste</span><span class="ax-sw"></span></button>';
  ...
  h += '<div class="ax-fam"><span class="ax-fam-h">Fonte de leitura</span>' +
       '<button type="button" class="f-padrao" data-font="padrao">Padr\u00E3o</button>' +
       '<button type="button" class="f-leitura" data-font="leitura">Leitura (mais leg\u00EDvel)</button>' +
       '<button type="button" class="f-dis" data-font="dis">OpenDyslexic</button></div>';
  painel.innerHTML = h;
  ...
  var elContraste = document.getElementById('axContraste');
  ...
  var elFamBtns = painel.querySelectorAll('.ax-fam button');

  /* ---------- pintar estados ---------- */
  function pintar() {
    if (elCalmo) { elCalmo.classList.toggle('ax-on', !!estado.calmo); elCalmo.setAttribute('aria-checked', estado.calmo ? 'true' : 'false'); }
    if (elContraste) { elContraste.classList.toggle('ax-on', !!estado.hc); elContraste.setAttribute('aria-checked', estado.hc ? 'true' : 'false'); }
  ...
  if (elContraste) elContraste.addEventListener('click', function () { estado.hc = !estado.hc; salvar(); aplicarContraste(); });
  ...
  for (var m = 0; m < elFamBtns.length; m++) {
    elFamBtns[m].addEventListener('click', function () {
      estado.font = this.getAttribute('data-font'); salvar(); aplicarFamilia();
    });
  }
